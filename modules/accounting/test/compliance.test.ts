...
describe('Accounting Module - Statutory Trust Accounting & Regulatory Compliance', () => {
  ...
  it('computes statutory Three-Way Bank Reconciliation with parity across GL trust, liability, and lease subledgers', () => {
    runInTenantContext('tenant-reconciliation-test', () => {
      ...
      const rec = AccountingRepository.getThreeWayReconciliation();
  ...
  it('aggregates annual vendor maintenance payments and flags IRS 1099-NEC $600 threshold', () => {
    runInTenantContext('tenant-1099-test', () => {
      const db = getDatabase();
      const now = Date.now();
      const taxYear = new Date().getUTCFullYear();
      const yearDate = Date.UTC(taxYear, 5, 15, 12, 0, 0); // June 15 of current year

      // Create two vendors: one qualifying (> $600) and one under threshold (< $600)
      ...
      // Record payments: Vendor 1 receives $1,250.00 (125,000 cents), Vendor 2 receives $350.00 (35,000 cents)
      ...
      assert.equal(report.tax_year, taxYear);
      assert.equal(report.threshold_cents, 60000, 'Threshold must be 60,000 cents ($600.00)');
      assert.equal(report.total_vendors_count, 2);
      assert.equal(report.qualifying_vendors_count, 1, 'Only Apex Plumbing should meet threshold');
      assert.equal(report.total_qualifying_payments_cents, 125000);
      ...
      assert.equal(qualifyingVendor.total_payments_cents, 125000);
      ...
      assert.equal(nonQualifyingVendor.total_payments_cents, 35000);
